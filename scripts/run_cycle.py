#!/usr/bin/env python3
"""Influencer Agent cycle runner — one full research/strategize cycle with git push.
...
def _call_api(
    ...
    if provider in ("anthropic", "anthropic-oauth"):
        ...
        if provider == "openrouter":
            headers["HTTP-Referer"] = "https://github.com/rookx88/autonomized-business-agent"
...
STATE_FILES = ["IDENTITY.md", "NICHE.md", "PERSONA.md", "VOICE.md", "AUDIENCE.md", "CONTENT_BANK.md", "STRATEGY.md", "JOURNAL.md", "LEARNINGS.md"]

APPEND_ONLY = {"JOURNAL.md", "LEARNINGS.md", "CONTENT_BANK.md"}
...
PHASE1_RUBRICS = {"niche-clarity", "persona-coherence", "platform-fit", "content-quality", "audience-targeting", "differentiation"}
PHASE2_RUBRICS = {"publishing-readiness"}
...
ASSESSOR_STATE_FILES = ["NICHE.md", "PERSONA.md", "VOICE.md", "AUDIENCE.md", "CONTENT_BANK.md", "STRATEGY.md"]
...
### ACTION-001
**Status:** PENDING
**Action:** Begin Phase 2 execution — find and contact first prospects
...
**Method:** Influencer Agent will identify 10 prospects from Reddit/Indie Hackers, draft personalised outreach for each, and propose sending via this approval queue.
**Cost estimate:** $0 (organic outreach only in Phase 2)
**Risk:** Low — no spend, no public commitment, just conversations
**Requested by:** Influencer Agent Generation {generation} (Phase 2 transition)
"""
...
## ⚠️ Cycle Focus: conversion-progress is critically low (0.10)
...
def _build_prompt(generation: int, state: dict[str, str], assessment: dict, skill_text: str) -> str:
    ...
    weakest = assessment.get("weakest_dimension", "")
    focus_block = CONVERSION_FOCUS_DIRECTIVE if weakest == "conversion-progress" else ""

    return f"""You are Influencer Agent, an autonomized business agent running generation {generation}.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one Influencer Agent business cycle")
    ...
    print(f"\n{'='*48}")
    print(f"  Influencer Agent — business cycle generation {next_gen}")
    print(f"{'='*48}\n")
    ...
    if args.push and not args.dry_run:
        commit_msg = f"Influencer Agent: generation {next_gen} — {skill_name} cycle (score {post['overall_score']:.2f})"
        print(f"\n→ Pushing to git...")
